...
public final class Select extends BodyTagImpl {
	...
	public void setMultiple(String multiple) {
		...
		if (engine.getCastUtil().toBooleanValue(multiple, true)) attributes.setEL("multiple", "multiple");
	}
	...
	public void setSize(double size) {
		attributes.setEL("size", engine.getCastUtil().toString(size));
	}
	...
	public void setQuery(String strQuery) throws PageException {
		this.query = engine.getCastUtil().toQuery(pageContext.getVariable(strQuery));
	}
	...
	public void setDataformatas(String dataformatas) throws PageException {
		...
		if ("plaintext".equals(lcDataformatas) || "html".equals(lcDataformatas)) {
			attributes.setEL("dataformatas", dataformatas);
				...
				.createApplicationException("attribute dataformatas for tag input has an invalid value [" + dataformatas + "], valid values are [plaintext, html");
	}
	...
	public void setDisabled(String disabled) {
		// alles ausser false ist true
		if (engine.getCastUtil().toBooleanValue(disabled, true)) setDisabled(true);
	}
	...
	public void setSelected(String selected) {
		this.selected = engine.getListUtil().trimItems(engine.getListUtil().toStringArray(selected, ","));
	}
	...
	private void _doEndTag() throws IOException, PageException {
		...
		if (query != null) {
			if (value == null)
				throw engine.getExceptionUtil().createApplicationException("if you have defined attribute query for tag select, you must also define attribute value");
			else if (!query.containsKey(value))
				throw engine.getExceptionUtil().createApplicationException("invalid value for attribute [value], there is no column in query with name [" + value + "]");

			if (display != null && !query.containsKey(display))
				throw engine.getExceptionUtil().createApplicationException("invalid value for attribute [display], there is no column in query with name [" + display + "]");

			if (group != null && !query.containsKey(group))
				throw engine.getExceptionUtil().createApplicationException("invalid value for attribute [group], there is no column in query with name [" + group + "]");
		}
		...
		if (parent instanceof Form) {
			...
			throw engine.getExceptionUtil().createApplicationException("Tag cfselect must be inside a cfform tag");
		}
		...
		if (query != null) {
			...
			for (int i = 1; i <= rowCount; i++) {
				v = engine.getCastUtil().toString(query.getAt(value, i));
				d = hasDisplay ? engine.getCastUtil().toString(query.getAt(display, i)) : v;
				if (hasGroup) {
					tmp = engine.getCastUtil().toString(query.getAt(group, i));
					if (currentGroup == null || !engine.getOperatonUtil().equals(currentGroup, tmp, true)) {
		...
		if (selected != null) {
			for (int i = 0; i < selected.length; i++) {
				if (caseSensitive) {
					...
					if (engine.getOperatonUtil().compare(str, selected[i]) == 0) return " selected";
				}
	 ...
	 * @throws PageException
	 */
	public void setQueryposition(String strQueryPosition) throws PageException {
		...
		if ("above".equals(strQueryPosition)) queryPosition = QUERY_POSITION_ABOVE;
				...
				.createApplicationException("attribute queryPosition for tag select has an invalid value [" + strQueryPosition + "], " + "valid values are [above, below]");
	...
	public void setEnabled(String enabled) {
		setDisabled(!engine.getCastUtil().toBooleanValue(enabled, true));
	}
