 ...
 * 
 **/
package org.lucee.extension.form.tag.jakarta;
...
public class Input extends TagImpl {
	 ...
	 */
	public void setValidate(String validate) throws PageException {
		...
		else throw engine.getExceptionUtil().createApplicationException("attribute validate has an invalid value [" + validate + "]",
	...
	public void setDir(String dir) {
		...
		// else throw engine.getExceptionUtil().createApplicationException("attribute dir for tag input has
		// an invalid value ["+dir+"], valid values are [ltr, rtl]");
	}
	...
	public void setDataformatas(String dataformatas) {
		...
		// else throw engine.getExceptionUtil().createApplicationException("attribute dataformatas for tag
		// input has an invalid value ["+dataformatas+"], valid values are [plaintext, html");
	}

	public void setDisabled(String disabled) {
		// alles ausser false ist true
		if (engine.getCastUtil().toBooleanValue(disabled, true)) attributes.setEL("disabled", disabled);
	}

	public void setEnabled(String enabled) {
		// alles ausser false ist true
		// setDisabled(engine.getCastUtil().toString(!engine.getCastUtil().toBooleanValue(enabled,true)));
		attributes.setEL("enabled", enabled);
	}

	public void setIsmap(String ismap) {
		// alles ausser false ist true
		// if(engine.getCastUtil().toBooleanValue(ismap,true)) attributes.setEL("ismap","ismap");
		attributes.setEL("ismap", ismap);
	}

	public void setReadonly(String readonly) {
		// alles ausser false ist true
		// if(engine.getCastUtil().toBooleanValue(readonly,true)) attributes.setEL("readonly","readonly");
	...
	public void setMaxlength(double maxLength) {
		...
		attributes.setEL("maxLength", engine.getCastUtil().toString(maxLength));
	}
	...
	public void setChecked(String checked) {
		// alles ausser false ist true
		if (engine.getCastUtil().toBooleanValue(checked, true)) attributes.setEL("checked", "checked");
	}
	...
	public void setDaynames(String listDaynames) throws PageException {
		...
		if (arr.length != 7) throw engine.getExceptionUtil()
				.createApplicationException("value of attribute [daynames] must contain a string list with 7 values, now there are " + arr.length + " values");
		this.daynames = arr;
	}
	 ...
	 * @throws PageException
	 */
	public void setFirstdayofweek(double firstDayOfWeek) throws PageException {
		if (firstDayOfWeek < 0 || firstDayOfWeek > 6)
			throw engine.getExceptionUtil().createApplicationException("value of attribute [firstDayOfWeek] must conatin a numeric value between 0-6");
	 ...
	 * @throws PageException
	 */
	public void setMonthnames(String listMonthNames) throws PageException {
		String[] arr = engine.getListUtil().toStringArray(listMonthNames, ",");
		if (arr.length == 12) throw engine.getExceptionUtil()
				.createApplicationException("value of attribute [MonthNames] must contain a string list with 12 values, now there are " + arr.length + " values");
		this.monthnames = arr;
	}
	...
	public void setType(String type) throws PageException {
		...
		else throw engine.getExceptionUtil().createApplicationException("attribute type has an invalid value [" + type + "]",
	 ...
	 * @throws PageException
	 */
	public void setPattern(String pattern) throws PageException {
	...
	public void setRange(String range) throws PageException {
		...
		Array arr = engine.getListUtil().toArray(range, ",");

		if (arr.size() == 1) {
			double from = engine.getCastUtil().toDoubleValue(arr.get(1, null), Double.NaN);
			if (!engine.getDecisionUtil().isValid(from)) throw engine.getExceptionUtil().createApplicationException(errMessage, errDetail);
			input.setRangeMin(from);
			input.setRangeMax(Double.NaN);
		}
		else if (arr.size() == 2) {
			String strFrom = arr.get(1, "").toString().trim();
			double from = engine.getCastUtil().toDoubleValue(strFrom, Double.NaN);
			if (!engine.getDecisionUtil().isValid(from) && strFrom.length() > 0) {
				throw engine.getExceptionUtil().createApplicationException(errMessage, errDetail);
			}
			input.setRangeMin(from);

			String strTo = arr.get(2, "").toString().trim();
			double to = engine.getCastUtil().toDoubleValue(strTo, Double.NaN);
			if (!engine.getDecisionUtil().isValid(to) && strTo.length() > 0) {
				throw engine.getExceptionUtil().createApplicationException(errMessage, errDetail);
			}
			input.setRangeMax(to);

		}
		else throw engine.getExceptionUtil().createApplicationException(errMessage, errDetail);
	}
	...
	public void setMessage(String message) {
		if (!engine.getStringUtil().isEmpty(message)) input.setMessage(message);
	}
	...
	private void _doEndTag() throws PageException, IOException {
		...
		if (input.getValidate() == VALIDATE_REGULAR_EXPRESSION && input.getPattern() == null) {
			throw engine.getExceptionUtil().createApplicationException("when validation type regular_expression is seleted, the pattern attribute is required");
		}
		...
		if (parent instanceof Form) {
			...
			if (input.getType() == TYPE_DATEFIELD && form.getFormat() != Form.FORMAT_FLASH)
				throw engine.getExceptionUtil().createApplicationException("type [datefield] is only allowed if form format is flash");
		}
		else {
			throw engine.getExceptionUtil().createApplicationException("Tag must be inside a form tag");
		}
		draw();
	}
	...
	String enc(String str) {
		return engine.getHTMLUtil().escapeHTML(str, HTMLUtil.HTMLV20);
	}
	...
	private PageException notSupported(String label) {
		return engine.getExceptionUtil().createApplicationException("attribute [" + label + "] is not supported");
	}

	public void setAutosuggest(String autosuggest) throws PageException {
		throw notSupported("autosuggest");
		// attributes.setEL("bind",bind);
	}

	public void setAutosuggestbinddelay(double autosuggestBindDelay) throws PageException {
		throw notSupported("autosuggestBindDelay");
		// attributes.setEL("bind",bind);
	}

	public void setAutosuggestminlength(double autosuggestMinLength) throws PageException {
		throw notSupported("autosuggestMinLength");
		// attributes.setEL("bind",bind);
	}

	public void setBind(String bind) throws PageException {
		throw notSupported("bind");
		// attributes.setEL("bind",bind);
	}

	public void setBindattribute(String bindAttribute) throws PageException {
		throw notSupported("bindAttribute");
		// attributes.setEL("bind",bind);
	}

	public void setBindonload(boolean bindOnLoad) throws PageException {
	...
	public void setMaxresultsdisplayed(double maxResultsDisplayed) throws PageException {
		throw notSupported("maxResultsDisplayed");
		// attributes.setEL("bind",bind);
	}

	public void setOnbinderror(String onBindError) throws PageException {
		throw notSupported("onBindError");
		// attributes.setEL("bind",bind);
	}

	public void setShowautosuggestloadingicon(boolean showAutosuggestLoadingIcon) throws PageException {
		throw notSupported("showAutosuggestLoadingIcon");
		// attributes.setEL("bind",bind);
	}

	public void setSourcefortooltip(String sourceForTooltip) throws PageException {
	...
	public void setTypeahead(boolean typeahead) throws PageException {
